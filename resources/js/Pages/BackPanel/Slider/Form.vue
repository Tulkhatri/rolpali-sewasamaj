...
const props = defineProps({
    slider: Object, // Incoming prop from the server
});
const form = useForm({
    id: props?.slider?.id||null,
    title: props?.slider?.title||null,
    detail: props?.slider?.detail||null,
    image: null,
    preview: '/storage/' + props?.slider?.image??null,
});
const submit = () => {
    const res = form.post(route("admin.slider.save"), {
        onError: () => {
