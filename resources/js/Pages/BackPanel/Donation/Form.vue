...
const props = defineProps({
    donation: Object, // Incoming prop from the server
});
const form = useForm({
    id: props?.donation?.id||null,
    title: props?.donation?.title||null,
    detail: props?.donation?.detail||null,
    image: null,
    preview: '/storage/' + props?.donation?.image??null,
});
const submit = () => {
    const res = form.post(route("admin.donation.save"), {
        onError: () => {
        ...
        >
            ...
            <div class="right">
                <div class="button-wrapper flex justify-between gap-1">
                    ...
                    >
                        <Link :href="route('admin.donation')">
                            <font-awesome-icon
                                :icon="['fas', 'arrow-left']"
                                class="mr-2"
                            />
